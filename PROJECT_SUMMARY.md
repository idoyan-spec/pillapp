...
**גרסה:** `2026-09-04 14:10 v13 terms-gate` — מוצגת בתחתית כל מסך.
...
| `js/legal.js` | **מקור האמת** לתקנון: גרסה, נוסח, ומנגנון אישור |
| `TERMS.md` | התקנון כמסמך בריפו — נגזר אוטומטית מ-`js/legal.js` |
...
| תאריך | שינוי |
|--------|-------|
| 2026-09-04 | v13 — תקנון משפטי מקיף (19 סעיפים) ושער אישור חוסם בכניסה: חובה לגלול עד הסוף וגם לסמן, האישור נשמר עם גרסה וחותמת זמן, ושינוי גרסת תקנון ידרוש אישור מחדש. נוסף TERMS.md שנגזר אוטומטית מהמקור |
| 2026-09-04 | v12 — ערכת צבעים לפי לשון הפנייה: ורוד־סגול לנקבה, תכלת־כחול לזכר, כברירת מחדל. אפשר לבחור ידנית. עובד גם במצב כהה, ו-theme-color של סרגל המערכת מתעדכן |
| 2026-09-04 | v11 — בחירת סגנון פנייה (לבבי/עדין/קליל/ענייני) עם לבבי כברירת מחדל וניסוח חדש לכל ההודעות; התזכורת מציגה את האריזה והכדור זה לצד זה והשם גדול יותר; חיתוך אוטומטי של תמונות לאזור הרלוונטי (Gemini bounding box, נמדד IoU 0.994); nav.js חוזק ונוסף לו אבחון |
| 2026-09-04 | v10 — מסך אישורים אחד (התראות/מצלמה/מיקום/תנועה) שנפתח אוטומטית בהפעלה ראשונה עם "אישור הכול"; קודם כל הרשאה נתבקשה במקום אחר וחלקן נשארו לא מאושרות. אחרי אישור התראות מופעלות תזכורות השרת לבד |
| 2026-09-04 | v9 — כפתור "חזור" סוגר שכבה במקום את האפליקציה; הזנת שעה בפעולה אחת (כפתור "הוספה" הוסר); ריפוי הדחיפות נותק מ-sync() ומדגל enabled אחרי שהתגלה שהוא מעולם לא רץ; נוסף /api/diag לאבחון מהמכשיר |
| 2026-09-03 | v8 — באג חמור: מנוי דחיפה פג (FCM החזיר 410), השרת מחק אותו בשקט, והממשק המשיך להציג "פעיל" לנצח בזמן ששום תזכורת לא נשלחה. תוקן: השרת מסמן dead במקום למחוק, /api/status חושף זאת, האפליקציה מאמתת בכל פתיחה ומרפאה בעצמה (unsubscribe+subscribe), ו-pushsubscriptionchange ב-SW מחדש מנוי שפג |
| 2026-09-03 | v7 — התקנה למסך הבית בלחיצה מתוך האפליקציה (beforeinstallprompt נתפס אך לא נוצל קודם), עם הוראות ידניות לפי הדפדפן כגיבוי |
| 2026-09-03 | v6 — משוב: לא נמצא מקום למחוק תרופה, לא נמצא כפתור ההתראות, ו-Push מעולם לא הופעל (השרת הראה 0 מנויים). תוקן: כפתור מחיקה ברשימה ובכותרת העורך, באנר אדום במסך הראשי להפעלת התזכורות, רשימת "נשאר להגדיר" בראש ההגדרות, ומדריך מודרך למפתח Gemini |
| 2026-09-03 | v5 — שרת Web Push (Cloudflare Worker + cron כל דקה): התראה עם תמונת הכדור, כפתורי לקחתי/נודניק בתוך ההתראה, ונדנוד — הכול גם כשהאפליקציה סגורה והטלפון נעול. הצפנה נכתבה מאפס ואומתה מול http_ece |
| 2026-09-03 | v4 — דווח שתזכורת 08:00 לא הגיעה. סיבה: המנוע רץ בתוך הדף והדף היה סגור. נוסף: השלמת פערים ללא תקרת זמן, כרטיס "לא סומנו" בראש המסך, ניסוח נכון למנה ישנה (לא "קח עכשיו"), לוח אבחון עם "מתי רצה לאחרונה", ייצוא ליומן הטלפון (.ics), ותיקון מלכודת מטמון ש-הריצה קוד ישן |
