...
המפתחות אינם נשמרים בקובץ, אינם נכנסים ל-repo, ואינם ניתנים לקריאה חזרה מ-Cloudflare.

**אין להם גיבוי חיצוני** — חשבון המכונה ב-Bitwarden Secrets Manager הוא לקריאה בלבד
ואינו יכול ליצור סודות (נבדק: `bws secret create` מחזיר 404). זו החלטה מודעת ולא השמטה:
העלות של אובדן המפתח נמוכה. אם הוא אובד, מייצרים זוג חדש, מריצים שוב את שתי פקודות
ה-`secret put` ואת `wrangler deploy`, ואז בכל מכשיר לוחצים פעם אחת
**הגדרות ← תזכורות כשהאפליקציה סגורה ← הפעלה**. אין אובדן נתונים — הכול מקומי.

מי שרוצה גיבוי בכל זאת: להריץ את פקודת ה-`node -e` שלמעלה ולשמור את שתי השורות
ידנית ב-Bitwarden דרך הדפדפן.
